...
const currentUser = storedUsers.find(user => user.id === currentUserId);
...
function showScoreDEtails(level,category){
    ...
    const levelData= currentUser.levels.find(l=>l.level===level);
    const categoryData= levelData[category];

    document.getElementById('username').innerHTML=currentUser.username;
    document.getElementById('niveau').innerHTML=levelData.level;
    ...
    document.getElementById('moyenne').innerText=`${categoryData.noteCat/10*100}%`;
    document.getElementById('cercle_level').style.background=`conic-gradient(#525CEB 0% ${categoryData.noteCat/10*100}%, #BFCFE7 ${categoryData.noteCat/10*100}% 100%)`;
